...
const out = new URL("../docs/research/acceptance/safety-ui-2026-09-08/", import.meta.url);
await fs.mkdir(out, { recursive: true });
try {
  await page.goto(`${process.env.CODEX_CLOUD_SAFETY_UI_URL || "http://127.0.0.1:5174/"}#/agent`);
  ...
  await page.goto("http://127.0.0.1:5174/#/project/sample-app/thread/sample-app-1");
  const composer = page.getByPlaceholder("向云端 Codex 发送消息");
  ...
  console.log(JSON.stringify({ ok: true, checks: ["跨项目文件保护", "延迟上传归属", "草稿冲突保留与解决", "桌面与移动端无横向溢出"], screenshots: out.pathname }, null, 2));
} finally {
  releaseUpload(); await browser.close();
}
