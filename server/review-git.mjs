// Adapted from friuns2/codexui src/server/reviewGit.ts (MIT). See docs/research/source-notes.md.
...
async function runCommandResult(command, args, options = {}) {
  return await new Promise((resolve2, reject) => {
    ...
    proc.on("close", (code) => {
      if (timeout) clearTimeout(timeout);
      const normalizedStderr = timedOut ? [stderr.trim(), `Timed out after ${timeoutMs}ms`].filter(Boolean).join("\n") : stderr.trim();
      resolve2({
...
function parseGitHubSlugFromUrl(value) {
  const text = String(value || "").trim();
  const match =
    text.match(/github\.com[:/]([^/\s]+)\/([^/\s#?]+?)(?:\.git)?(?:\/pull\/(\d+))?(?:[?#].*)?$/iu) ||
    text.match(/^([^/\s]+)\/([^/\s#?]+?)(?:\.git)?$/u);
  if (!match) return null;
  return {
    owner: match[1],
    name: match[2].replace(/\.git$/iu, ""),
    number: match[3] ? Number(match[3]) : null
  };
}
function normalizeGitHubRepoInfo(repoView, prView, remoteUrl) {
  const remote = parseGitHubSlugFromUrl(remoteUrl);
  const prUrl = parseGitHubSlugFromUrl(prView?.url || "");
  const owner =
    readString(repoView?.owner?.login) ||
    readString(repoView?.owner?.name) ||
    readString(repoView?.owner) ||
    prUrl?.owner ||
    remote?.owner ||
    "";
  const name =
    readString(repoView?.name) ||
    readString(prView?.baseRepository?.name) ||
    readString(prView?.headRepository?.name) ||
    prUrl?.name ||
    remote?.name ||
    "";
  return {
    owner,
    name,
    url: readString(repoView?.url) || (owner && name ? `https://github.com/${owner}/${name}` : "")
  };
}
function reviewPrUnavailable(reason, patch = {}) {
  return {
    available: false,
    reason,
    ghInstalled: false,
...
async function detectReviewPullRequest(cwd) {
  ...
  if (!repoRoot) {
    return reviewPrUnavailable("这个目录还不是 Git 仓库");
  }
  let ghVersion;
  try {
    ghVersion = await runCommandResult("gh", ["--version"], { cwd: repoRoot, timeoutMs: 5_000 });
  } catch {
    return reviewPrUnavailable("云端没有可用的 GitHub CLI");
  }
  if (ghVersion.code !== 0) {
    return reviewPrUnavailable("云端没有可用的 GitHub CLI");
  }
  const [remoteResult, repoViewResult, authResult] = await Promise.all([
    runCommandResult("git", ["remote", "get-url", "origin"], { cwd: repoRoot }),
    runCommandResult("gh", ["repo", "view", "--json", "owner,name,url"], { cwd: repoRoot, timeoutMs: 8_000 }),
    runCommandResult("gh", ["auth", "status"], { cwd: repoRoot, timeoutMs: 8_000 })
  ]);
  const remoteUrl = remoteResult.code === 0 ? remoteResult.stdout : "";
  const repoView = repoViewResult.code === 0 ? parseJsonOutput(repoViewResult.stdout) : {};
  const repo = normalizeGitHubRepoInfo(repoView, {}, remoteUrl);
  if (authResult.code !== 0) {
    return reviewPrUnavailable("GitHub CLI 尚未登录", {
      ghInstalled: true,
      authenticated: false,
      repo: repo.owner && repo.name ? repo : null
    });
  }
  const prResult = await runCommandResult(
    "gh",
    ["pr", "view", "--json", "number,url,title,headRefName,baseRefName,headRefOid,state,headRepository,headRepositoryOwner"],
    { cwd: repoRoot, timeoutMs: 8_000 }
  );
  if (prResult.code !== 0) {
    return reviewPrUnavailable("当前分支没有关联的 GitHub PR", {
      ghInstalled: true,
      authenticated: true,
      repo: repo.owner && repo.name ? repo : null,
      detail: [prResult.stderr, prResult.stdout].filter(Boolean).join("\n")
    });
  }
  const prView = parseJsonOutput(prResult.stdout);
  const normalizedRepo = normalizeGitHubRepoInfo(repoView, prView, remoteUrl);
  const number = Number(prView.number || 0);
  if (!normalizedRepo.owner || !normalizedRepo.name || !number) {
    return reviewPrUnavailable("无法识别 GitHub 仓库或 PR 编号", {
      ghInstalled: true,
      authenticated: true,
      repo: normalizedRepo.owner && normalizedRepo.name ? normalizedRepo : null
    });
  }
  return {
    available: true,
    ...
    pr: {
      number,
      url: readString(prView.url),
      title: readString(prView.title),
      state: readString(prView.state),
      headRefName: readString(prView.headRefName),
      baseRefName: readString(prView.baseRefName),
      headRefOid: readString(prView.headRefOid)
    }
  };
}
async function publishReviewPullRequestComment(payload) {
  const record = asRecord(payload);
  if (!record) {
    throw new Error("Invalid body: expected object");
  }
  const cwd = readString(record.cwd);
  const body = readString(record.body).slice(0, 60_000);
  const filePath = readString(record.path);
  const startLine = Number(record.startLine || 0);
  const endLine = Number(record.endLine || startLine || 0);
  if (!cwd) throw new Error("Missing cwd");
  if (!body) throw new Error("Missing comment body");
  if (filePath && !isSafeGitRelativePath(filePath)) {
    throw new Error("Invalid review path");
  }
  const context = await detectReviewPullRequest(cwd);
  if (!context.available || !context.repo || !context.pr) {
    throw new Error(context.reason || "当前分支没有可发布的 PR");
  }
  if (filePath && Number.isFinite(endLine) && endLine > 0 && context.pr.headRefOid) {
    const args = [
      "api",
      `repos/${context.repo.owner}/${context.repo.name}/pulls/${context.pr.number}/comments`,
      "-f",
      `body=${body}`,
      "-f",
    ...
    if (Number.isFinite(startLine) && startLine > 0 && startLine < endLine) {
      args.push("-F", `start_line=${startLine}`, "-f", "start_side=RIGHT");
    }
    const result = await runCommandResult("gh", args, { cwd: normalizeInputCwd(cwd), timeoutMs: 20_000 });
    if (result.code !== 0) {
      const details = [result.stderr, result.stdout].filter(Boolean).join("\n");
      throw new Error(details || "GitHub PR inline comment failed");
    }
    const created = parseJsonOutput(result.stdout);
    return {
      ...context,
      published: true,
      mode: "inline",
      url: readString(created.html_url) || readString(created.url) || context.pr.url
    };
  }
  const result = await runCommandResult("gh", ["pr", "comment", String(context.pr.number), "--body", body], { cwd: normalizeInputCwd(cwd), timeoutMs: 20_000 });
  if (result.code !== 0) {
    const details = [result.stderr, result.stdout].filter(Boolean).join("\n");
    throw new Error(details || "GitHub PR comment failed");
  }
  return {
    ...context,
    published: true,
    mode: "conversation",
    url: result.stdout || context.pr.url
  };
}
...
async function summarizeUntrackedFile(repoRoot, path) {
  ...
  const addedLineCount = await new Promise((resolve2, reject) => {
    ...
    stream.on("end", () => {
      resolve2(sawAnyByte && !lastByteWasNewline ? lineCount + 1 : lineCount);
    });
  });
...
function buildReviewSnapshotFromDiff({
  cwd,
  gitRoot = cwd,
  diffText = "",
  scope = "workspace",
  ...
  const normalizedCwd = normalizeInputCwd(cwd);
  const normalizedGitRoot = gitRoot ? normalizeInputCwd(gitRoot) : normalizedCwd;
  const files = parseReviewSnapshotFiles(normalizedGitRoot, diffText);
  return {
    cwd: normalizedCwd,
...
async function buildReviewSummary(cwd, workspaceView) {
  ...
  const diffText = await buildWorkspaceDiff(gitRoot, workspaceView);
  const files = parseReviewSnapshotFiles(gitRoot, diffText);
  return {
    fileCount: files.length,
    addedLineCount: files.reduce((sum, file) => sum + file.addedLineCount, 0),
...
async function handleReviewRoutes(req, res, url, context) {
  ...
  if (req.method === "GET" && url.pathname === "/codex-api/review/pr-context") {
    const cwd = url.searchParams.get("cwd")?.trim() ?? "";
    if (!cwd) {
      setJson(res, 400, { error: "Missing cwd" });
      return true;
    ...
    try {
      setJson(res, 200, {
        data: await withDeadline(detectReviewPullRequest(cwd), 15_000, "PR 状态读取超时")
      });
    } catch (error) {
      setJson(res, 500, { error: getErrorMessage(error, "Failed to read PR context") });
    }
    return true;
  }
  ...
  if (req.method === "POST" && url.pathname === "/codex-api/review/pr-comment") {
    try {
      const payload = await context.readJsonBody(req);
      setJson(res, 200, {
        data: await withDeadline(publishReviewPullRequestComment(payload), 35_000, "PR 评论发布超时")
      });
    } catch (error) {
      setJson(res, 500, { error: getErrorMessage(error, "Failed to publish PR comment") });
    }
    return true;
  }
