...
侧栏顶部可在“个人 / 工作”之间切换。个人助理默认复用工作空间已登录的 Codex 账号，不需要再登录；目录、对话、草稿及默认搜索范围独立，切换后回到各自的对话，不自动把工作对话拼入个人上下文。共用账号也共用额度与全局 Codex 配置，这不是系统用户或文件访问权限的保密隔离。需要独立账号/权限时，可选择[专用 worker](docs/setup.md#个人空间专用-worker)，它不是日常个人使用的前提。

- **从具体任务开始**：新对话提供调研、邮件待办、日程安排和文档整理入口；已有对话可点“任务建议”。选择后只填入草稿，不会自动发送或消耗模型额度。
- **保存文件**：默认只读。点击输入框下的权限，选择“允许写入个人工作区”，确认后后续消息可保存产物，额外权限仍按需审批。个人空间不提供全主机权限；正在运行时不能切换权限。
