...
侧栏顶部可在“个人 / 工作”之间切换。个人助理默认复用工作空间已登录的 Codex 账号，不需要再登录；目录、对话、草稿及默认搜索范围独立，切换后回到各自的对话。当前个人任务保持只读与逐项审批，不自动把工作对话拼入个人上下文。共用账号也共用额度与全局 Codex 配置，这不是系统用户或文件访问权限的保密隔离。需要独立账号/权限时，可选择[专用 worker](docs/setup.md#个人空间专用-worker)，它不是日常个人使用的前提。
